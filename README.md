...
downloaded and stored by us, and the contributor credit is kept alongside it and
shown wherever the photo appears — that attribution is a condition of using
their API.

The JPEG lives in a `bytea` column and is served by
`GET /api/v1/trips/{id}/image`. At roughly 70 KB a photo, a thousand trips is
under 70 MB, which is far cheaper than standing up object storage and needs no
bucket, IAM policy or extra credentials. That endpoint is intentionally
unauthenticated — a browser cannot attach a Bearer token to an `<img src>`, and
the payload is a royalty-free stock photo of a city.

Set `PIXABAY_API_KEY` to enable it. Leave it unset and the lookup is skipped:
trips are created normally, just without a photo. The lookup runs after the trip
is committed and swallows every error, so it can never cost you a trip.
